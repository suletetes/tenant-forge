# prod environment — thin wrapper over the shared stack (Task 19). Deployed only after staging
# passes and a manual approval on the GitHub `production` environment (cd.yml, R18.4).
...
terraform {
  ...
  backend "s3" {
    key = "tenantforge/prod/terraform.tfstate"
  }
}
